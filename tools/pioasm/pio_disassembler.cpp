...
extern "C" void disassemble(char *buf, int buf_len, uint16_t inst,
                            uint sideset_bits, bool sideset_opt) {
  if (buf_len)
    buf[disassemble(inst, sideset_bits, sideset_opt).copy(buf, buf_len - 1)] =
        0;
}
...
std::string disassemble(uint16_t inst, uint sideset_bits_including_opt,
  ...
  switch (major) {
    case 0b000: {
      static std::array<std::string, 8> conditions{
    ...
    case 0b010: {
      static std::array<std::string, 8> sources{"pins", "x",      "y",   "null",
                                                "",     "status", "isr", "osr"};
    ...
    case 0b011: {
      static std::array<std::string, 8> dests{"pins",    "x",  "y",   "null",
                                              "pindirs", "pc", "isr", "exec"};
      op("out");
    ...
    case 0b101: {
      static std::array<std::string, 8> dests{"pins", "x",  "y",   "",
                                              "exec", "pc", "isr", "osr"};
      static std::array<std::string, 8> sources{"pins", "x",      "y",   "null",
                                                "",     "status", "isr", "osr"};
